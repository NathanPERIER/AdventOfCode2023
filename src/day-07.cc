...
public:
	...
	bool operator<(const card_hand& other) const {
		if(_type != other._type) {
			return static_cast<uint8_t>(_type) < static_cast<uint8_t>(other._type);
		}
		for(size_t i = 0; i < 5; i++) {
			if(_cards[i] != other._cards[i]) {
				return compare_cards(_cards[i], other._cards[i]);
			}
		}
		return false; // both hands are equal
	}

	friend std::ostream& operator<<(std::ostream& os, const card_hand& hand);
...
private:
	...
	static hand_type compute_type(const std::string& cards) {
		...
		const uint8_t max_count = it->second;
		if(counter.size() == 2) {
			return (max_count == 4) ? hand_type::four_of_a_kind : hand_type::full_house;
		}
		// counter.size() == 3
		return (max_count == 3) ? hand_type::three_of_a_kind : hand_type::two_pair;
	}

};

std::ostream& operator<<(std::ostream& os, const card_hand& hand) {
    os << hand._cards << " " << hand._value;
    return os;
}


uint64_t winnings(std::vector<card_hand>& hands) {
	uint64_t res = 0;
